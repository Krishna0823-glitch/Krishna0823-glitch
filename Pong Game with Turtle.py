...
ball.penup()
ball.goto(0, 0)
ball.dx = 0.1  # Ball movement in the x direction
ball.dy = -0.1  # Ball movement in the y direction
...
def paddle_a_up():
    y = paddle_a.ycor()
    if y < 250:  # Check to prevent the paddle from going off the screen
        y += 20
    paddle_a.sety(y)
...
def paddle_a_down():
    y = paddle_a.ycor()
    if y > -240:  # Check to prevent the paddle from going off the screen
        y -= 20
    paddle_a.sety(y)
